...
describe("GuildCredential", () => {
  ...
  context("#tokenURI", () => {
    ...
    it.skip("should return the correct tokenURI", async () => {
      /* eslint-disable no-await-in-loop */
      for (let i = 0; i < cids.length; i += 1) {
        await credential.claim(constants.AddressZero, GuildAction.JOINED_GUILD, 1985 + i, cids[i], { value: fee }); // TODO
  ...
  context.skip("#claim", () => {
    ...
    it("emits Claimed event", async () => {
      await expect(credential.claim(mockERC20.address, GuildAction.IS_ADMIN, 1985, cids[0]))
